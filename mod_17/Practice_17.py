...
# Реализуем функцию, в которой устанавливается номер позиции элемента,
...
def binary_search(array_int, element, left, right):
    ...
    if min(array_int) >= element:
        return print('Введенное число меньше или равно минимальному элементу в списке')

    middle = ((right + left) // 2)   # находим середину и округлим в большую сторону при нечетном кол-ве
    if array_int[middle] >= element and array_int[middle-1] < element:
...
element_pos = binary_search(array_sort, user_element, 0, len(array_sort))
if element_pos is not None: # нельзя просто писать If element_pos. Там мы приводим переменную к Bool и None = 0
    print('Номер позиции элемента:', element_pos)
